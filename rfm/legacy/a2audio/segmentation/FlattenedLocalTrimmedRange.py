...
@AudioSegmenter.declare_type("flattened-local-trimmed-range", "fltr")
class FlattenedLocalTrimmedRangeAudioSegmenter(AudioSegmenter):
    def compute_mask(self, spectrum, storage, **params):
        ...
        if th == 'yen':
            th = skimage.filters.threshold_yen(Sdb)
            Smask = Srange > th
        else:
            Smask = Sdb > th
